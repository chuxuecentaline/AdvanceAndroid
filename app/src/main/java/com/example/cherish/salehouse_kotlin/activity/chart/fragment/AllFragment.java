...
import com.example.cherish.salehouse_kotlin.activity.chart.CacheUtils;
...
public class AllFragment extends BaseChartFragment {
    ...
    protected void requestData() {
        HttpUtils.with(getActivity()).url(HttpApi.trend).get().execute(new EngineCallBack() {
            ...
            public void onSuccess(String result) {
                ...
                //按照时间进行排序
                Collections.sort(gScopeDealAvgPriceHistory, new
                        CollectionsUtils<GScopeDealAvgPriceHistoryBean>());
                ...
                System.out.println("mLeftAxis"+mLeftAxis.toString());
                List<String> axis = CacheUtils.getInstance().getData();
                if(axis==null||axis.size()==0){
                    CacheUtils.getInstance().setData(mLeftAxis);
                }
